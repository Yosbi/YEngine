...
public:
	...
	// Change technique in a FX
	virtual void	ChangeFXTechnique(UINT nFXID, const char *chTechnique) = 0;
	...
	// Activate/deactivate color buffer
	// If The color buffer is deactivated,sets the alphablend = true, srcblend = 0, destblend = 1	
	virtual void	UseColorBuffer	(bool b) = 0;

	// Return true if you are using color buffer
	virtual bool    UsesColorBuffer	(void)  = 0;

	// Activate additive blending, sets the alphablend = true, srcblend = 1, destblend = 1
	virtual void	UseAdditiveBlending(bool b) = 0;

	// Return true if are using additive blending
	virtual bool    UsesAdditiveBlending(void) = 0;

	//// Activate alpha blending, forces the engine to set the alphablend = true, srcblend = srcalpha, destblend = invsrcalpha
	//virtual void    UseAlphaBlending(bool b) = 0;

	//// Return true if using alpha blending
	//virtual bool	UsesAlphaBlending(void) = 0;



	// Set the backface culling
	virtual void	SetBackfaceCulling(YRENDERSTATE rs) = 0;

	// Set a depth stencil buffer state
	virtual void	SetStencilBufferMode(YRENDERSTATE rs, DWORD dw) = 0;

	// Activate or deactivate depth buffer with the values READONLY, READWRITE or NONE
	virtual void	SetDepthBufferMode(YRENDERSTATE rs) = 0;
	...
	//---------------------------
	// Mesh stuff
	// Desc: To create generic test object, dont render with the vertexCacheManager but it own render function MeshRender()
	//---------------------------

	// Create teapot
	virtual HRESULT MeshCreateTeapot(UINT *nMID) = 0;

	// Create a sphere
	virtual HRESULT MeshCreateSphere(float fRadius, UINT nSlices, UINT nStacks, UINT *nMID) = 0;

	// Create a cylinder
	virtual HRESULT MeshCreateCylinder(float fRadius1, float fRadius2, float fLength, UINT nSlices, UINT nStacks, const char cSkinOrientation, UINT *nMID) = 0;

	// Render the mesh
	virtual HRESULT MeshRender(UINT nMID, UINT nSkinID) = 0;
...
protected:
	...
	bool					m_bAlpha;			// Using alpha blending?

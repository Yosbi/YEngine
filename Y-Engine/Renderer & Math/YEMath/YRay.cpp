...
// Desc: Set attributs
//--------------------------------------------------------------------
inline void YRay::Set(YVector vcOrig, YVector vcDir) {
   m_vcOrig = vcOrig;
   m_vcDir  = vcDir;
}
...
//--------------------------------------------------------------------
// Name: DeTransform()
// Desc: transform ray into matrix space
//--------------------------------------------------------------------
inline void YRay::DeTransform(const YMatrix &_m) {
   YMatrix mInv;
   YMatrix m = _m;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Test for intersection with triangle
//--------------------------------------------------------------------
bool YRay::Intersects(const YVector &vc0, const YVector &vc1, 
                        const YVector &vc2, bool bCull, float *t) {
   YVector pvec, tvec, qvec;

   YVector edge1 = vc1 - vc0;
   YVector edge2 = vc2 - vc0;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: test for intersection with triangle at certain length (line segment),
//		same as above but test distance to intersection vs segment length.
//--------------------------------------------------------------------
bool YRay::Intersects(const YVector &vc0, const YVector &vc1, 
                        const YVector &vc2, bool bCull, float fL, float *t) {
   YVector pvec, tvec, qvec;
...
//--------------------------------------------------------------------
bool YRay::Intersects(const YAabb &aabb, float *t) {
   bool bInside = true;
   float t0, t1, tmp;
...
//		from "Geometric Tools...", Morgan Kaufmann Publ., 2002
//--------------------------------------------------------------------
bool YRay::Intersects(const YAabb &aabb, float fL, float *t) {
   bool bInside = true;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Test for intersection with obb, slaps method
//--------------------------------------------------------------------
bool YRay::Intersects(const YObb &obb, float *t) {
   float e, f, t1, t2, temp;
   float tmin = -99999.9f, 
         tmax = +99999.9f;

   YVector vcP = obb.vcCenter - m_vcOrig;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Test for intersection with obb at certain length (line segment),
//		slaps method but compare result if true to length prior return.
//-------------------------------------------------------------------- 
bool YRay::Intersects(const YObb &obb, float fL, float *t) {
   float e, f, t1, t2, temp;
   float tmin = -99999.9f, 
         tmax = +99999.9f;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Intersection with Plane from origin till infinity.
//--------------------------------------------------------------------  
bool YRay::Intersects(const YPlane &plane, bool bCull, 
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Intersection with Plane at distance fL. 
//--------------------------------------------------------------------  
bool YRay::Intersects(const YPlane &plane, bool bCull, float fL,
                        float *t, YVector *vcHit) {
   float Vd = plane.m_vcN * m_vcDir;

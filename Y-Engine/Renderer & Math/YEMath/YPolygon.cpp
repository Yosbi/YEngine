...
// Desc: Constructor
//--------------------------------------------------------------------
YPolygon::YPolygon(void) {
   m_pPoints = NULL;
   m_pIndis  = NULL;
   m_NumP    = 0;
   m_NumI    = 0;
   m_Flag    = 0;
   memset(&m_Aabb, 0, sizeof(YAabb));
} 
...
// Desc: Sets a polygon
//--------------------------------------------------------------------
void YPolygon::Set(const YVector *pPoints, int nNumP,
   ...
   m_pPoints = new YVector[nNumP];
   m_pIndis  = new unsigned int[nNumI];

   m_NumP = nNumP;
   m_NumI = nNumI;

   memcpy(m_pPoints, pPoints, sizeof(YVector)*nNumP);
...
//--------------------------------------------------------------------
// Name: CalcBoundingBox()
// Desc: Calculate the bounding box of that poligon
//--------------------------------------------------------------------
void YPolygon::CalcBoundingBox(void) {
   YVector vcMax, vcMin;
...
//--------------------------------------------------------------------
// Name: CalcBoundingBox()
// Desc: Change frontside to backside and vice versa
//--------------------------------------------------------------------
void YPolygon::SwapFaces(void) {
...
//--------------------------------------------------------------------
// Name: Clip()
// Desc: Split this poly with the given plane, returns two new polys
// on front and back side of the plane.
//-------------------------------------------------------------------- 
void YPolygon::Clip(const YPlane &Plane, YPolygon *pFront, 
                      YPolygon *pBack) {
   if (!pFront && !pBack) return;

   YVector vcHit, vcA, vcB;
   YRay    Ray;

   // cast away const
   YPlane  *pPlane = ((YPlane*)&Plane);
   ...
   YVector *pvcFront = new YVector[m_NumP*3];
   YVector *pvcBack  = new YVector[m_NumP*3];
...
//--------------------------------------------------------------------
// Name: Clip()
// Desc: Clips the poly that it fits into the given aabb
//-------------------------------------------------------------------- 
void YPolygon::Clip(const YAabb &aabb) {
   YPolygon BackPoly, ClippedPoly;
   YPlane   Planes[6];
   bool     bClipped=false;

   // cast away const
   YAabb *pAabb = ((YAabb*)&aabb);
   ...
   for (int i=0; i<6; i++) {
      if ( Planes[i].Classify(ClippedPoly) == YCLIPPED ) {
...
//--------------------------------------------------------------------
// Name: Cull()
// Desc: Checks if the polygon is inside, outisde or clipped by the aabb
// CLIPPED CASE ONLY APPROXIMATED
//-------------------------------------------------------------------- 
int YPolygon::Cull(const YAabb &aabb) {
   YPlane   Planes[6];
   int      nClass=0;
   ...
   for (int p=0; p<6; p++) {
      ...
      if (bFirst) {
         ...
         if (nInside  == m_NumP) return YVISIBLE;
         }
...
//--------------------------------------------------------------------
// Name: CopyOf()
// Desc: Makes this polygon a copy of the given polygon
//-------------------------------------------------------------------- 
void YPolygon::CopyOf(const YPolygon &Poly) {
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Intersection with a ray
//-------------------------------------------------------------------- 
bool YPolygon::Intersects(const YRay &Ray, bool bCull, float *t) {
   // cast away const
   YRay *pRay = (YRay*)&Ray;
...
//--------------------------------------------------------------------
// Name: Intersects()
// Desc: Intersection with a line segment
//-------------------------------------------------------------------- 
bool YPolygon::Intersects(const YRay &Ray, bool bCull, float fL, float *t) {
   // cast away const
   YRay *pRay = (YRay*)&Ray;
...
//--------------------------------------------------------------------
// Name: Print()
// Desc: Output polygon data to textfile
//-------------------------------------------------------------------- 
void YPolygon::Print(FILE *p) {
   if (!p) return;

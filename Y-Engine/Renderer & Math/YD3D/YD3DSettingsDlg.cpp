...
LPCTSTR D3DFormatToString( D3DFORMAT format, bool bWithPrefix )
{
    ...
    // _tcsstr is for find a substring
...
//-----------------------------------------------------------------------------
// Name: CD3DSettingsDlg () 
// Desc: CD3DSettingsDlg Class Constructor
//-----------------------------------------------------------------------------
YD3DSettingsDlg::YD3DSettingsDlg()
{
...
//-----------------------------------------------------------------------------
// Name: ~CD3DSettingsDlg () 
// Desc: CD3DSettingsDlg Class Destructor
//-----------------------------------------------------------------------------
YD3DSettingsDlg::~YD3DSettingsDlg()
{
	// Clean up
...
//-----------------------------------------------------------------------------
int YD3DSettingsDlg::ShowDialog( YD3DInitialize * pInitialize, YD3DSettings *pSettings, HWND hWndParent,HINSTANCE hInstParent )
{
    ...
    {
        ZeroMemory( &m_D3DSettings, sizeof(YD3DSettings) );
	...
	if((ret = DialogBoxParam( hInstParent, "dlgChangeDevice" , hWndParent, StaticDlgProc, (LPARAM)this )) == -1)
	{
		MessageBox(NULL, "aaaaayayaya!", "a", MB_OK);
	}
	return ret;
	//return DialogBox( NULL, MAKEINTRESOURCE( IDD_D3DSETTINGS ) , NULL, StaticDlgProc );
}

//-----------------------------------------------------------------------------
// Name: StaticDlgProc () (Static Callback)
// Desc: This is the main messge proc for all settings dialogs, it captures
...
//		  It is VITALLY important pass this 'this' pointer to
//        the lpParam parameter of the CreateDialogParam function  
//        to be able to pass messages back to that settings object.
//-----------------------------------------------------------------------------
BOOL CALLBACK YD3DSettingsDlg::StaticDlgProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
     //If this is a create message, trap the 'this' pointer passed in the lparam and store it within the window.
    if ( Message == WM_INITDIALOG ){
		SetWindowLong( hWnd, GWL_USERDATA, lParam);
	}
	
    //obtain the correct destination for this message
    YD3DSettingsDlg *Destination = (YD3DSettingsDlg*)GetWindowLong( hWnd, GWL_USERDATA );
...
//-----------------------------------------------------------------------------
BOOL CALLBACK YD3DSettingsDlg::SettingsDlgProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ULONG i;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::AdapterChanged(  )
{
    YD3DEnumAdapter * pAdapter = (YD3DEnumAdapter*)Combo_GetSelectedItemData( IDC_COMBO_ADAPTER );
    ...
    {
        YD3DEnumDevice *pDevice = pAdapter->Devices[i];
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::DeviceChanged(  )
{
    YD3DEnumDevice * pDevice = (YD3DEnumDevice*)Combo_GetSelectedItemData( IDC_COMBO_DEVICE );
    ...
    {
        YD3DEnumDeviceOptions * pDeviceOptions = (YD3DEnumDeviceOptions*)pDevice->Options[ i ];
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::WindowedFullscreenChanged(  )
{
    YD3DEnumAdapter * pAdapter = (YD3DEnumAdapter*)Combo_GetSelectedItemData( IDC_COMBO_ADAPTER );
    YD3DEnumDevice  * pDevice  = (YD3DEnumDevice *)Combo_GetSelectedItemData( IDC_COMBO_DEVICE );
    ...
    if ( IsDlgButtonChecked( m_hWndDlg, IDC_RADIO_WINDOWED ) )
    {
        ...
        _sntprintf_s( strResolution, 50, _T("%d x %d"), Width, Height );
        strResolution[49] = 0;
        ...
        _sntprintf_s( strRefreshRate, 50, _T("%d Hz"), ulRefreshRate );
        strRefreshRate[49] = 0;
    ...
    else
    {
        ...
        for ( ULONG i = 0; i < pDevice->Options.size(); i++ )
        {
            YD3DEnumDeviceOptions * pDeviceOptions = pDevice->Options[ i ];
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::AdapterFormatChanged(  )
{
    ULONG     i;
    D3DFORMAT AFormat;
    ...
    if ( m_D3DSettings.Windowed == false )
    {
        ...
        for ( i = 0; i < pAdapter->Modes.size(); i++ )
        {
            ...
            if ( Mode.Format == AFormat )
            {
                ...
                _sntprintf_s( strResolution, 50, _T("%d x %d"), Mode.Width, Mode.Height );
                strResolution[49] = 0;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::ResolutionChanged(  )
{
    ULONG i;
    ...
    for ( i = 0; i < pAdapter->Modes.size(); i++ )
    {
        ...
        {
            TCHAR strRefreshRate[50];
            _sntprintf_s( strRefreshRate, 50, _T("%d Hz"), Mode.RefreshRate );
            strRefreshRate[49] = 0;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::RefreshRateChanged( )
{
    if( m_D3DSettings.Windowed ) return;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::BackBufferFormatChanged( )
{
    ULONG i, j;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::DepthStencilFormatChanged( )
{
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::MultisampleTypeChanged( )
{
    ULONG               i;
    ...
    YD3DEnumDeviceOptions * pDeviceOptions = NULL;
    for ( i = 0; i < pDevice->Options.size(); i++ )
    {
    ...
    {
        TCHAR strQuality[50];
        _sntprintf_s( strQuality, 50, _T("%d"), i );
        strQuality[49] = 0;
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::MultisampleQualityChanged( )
{
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::VertexProcessingChanged( )
{
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::PresentIntervalChanged( )
{
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::Combo_ItemAdd( ULONG ComboID, LPVOID pData, LPCTSTR pStrDesc )
{
...
//-----------------------------------------------------------------------------
LPVOID YD3DSettingsDlg::Combo_GetSelectedItemData( ULONG ComboID )
{
    // Get the control handle
    HWND hwndCtrl = GetDlgItem( m_hWndDlg, ComboID );
...
//-----------------------------------------------------------------------------
ULONG YD3DSettingsDlg::Combo_GetItemCount( ULONG ComboID )
{
    // Get the control handle
    HWND hwndCtrl = GetDlgItem( m_hWndDlg, ComboID );
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::Combo_SelectItem( ULONG ComboID, ULONG Index )
{
    // Get the control handle
    HWND hwndCtrl = GetDlgItem( m_hWndDlg, ComboID );
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::Combo_SelectItemData( ULONG ComboID, LPVOID pData )
{
    // Get the control handle
    HWND hwndCtrl = GetDlgItem( m_hWndDlg, ComboID );
...
//-----------------------------------------------------------------------------
void YD3DSettingsDlg::Combo_Clear( ULONG ComboID )
{
    // Get the control handle
    HWND hwndCtrl = GetDlgItem( m_hWndDlg, ComboID );
...
//-----------------------------------------------------------------------------
bool YD3DSettingsDlg::Combo_FindText( ULONG ComboID, LPCTSTR pStrText )
{
    TCHAR strItem[255];

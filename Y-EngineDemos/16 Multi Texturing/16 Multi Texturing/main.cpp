...
HRESULT ProgramTick(void) 
{
	...
	g_pDevice->DrawText(g_nFontID, 55, 20, 0, 200, 0, "Y-Engine Demo: Ground Demo");
...
{	
	 return g_pDevice->CreateFX("MultiTex.fx", "MultiTexTech", true, NULL);		
} 
				...
				VERTEX *verts, WORD *indices, int *nIndis, int *nVerts)
{
	...
	*nIndis = numTris * 3;
	...
	g_pDevice->GetSkinManager()->AddTexture(z, "grass0.dds", false, 0.0f, NULL, 0);
	g_pDevice->GetSkinManager()->AddTexture(z, "stone2.dds", false, 0.0f, NULL, 0);
	g_pDevice->GetSkinManager()->AddTexture(z, "blendmap.jpg", false, 0.0f, NULL, 0);

...
HRESULT ProgramTick(void) 
{
	...
	g_pDevice->DrawText(g_nFontID, 55, 20, 0, 200, 0, "Y-Engine Demo: Gate Demo");
...
HRESULT Render(float TimeElapsed)
{
	...
	g_pDevice->SetViewLookAt(YVector(1,up, 0.2f * (-fBackForw)) * vmat, YVector(0,0,0), YVector(0,1,0));
	...
	YVector vcLight	= YVector(0.0f, 1.0f, -1.0f);
	vcLight.Normalize();
	...
	mat.TransposeOf(mat);
	g_pDevice->SetFXConstant(DAT_MATRIX, "gWorldInverseTranspose", &mat, 0);
	g_pDevice->GetVertexManager()->Render(g_sGate);
	g_pDevice->GetVertexManager()->Render(g_sGate2);
	return Y_OK;
}
...
HRESULT BuildGeometry(void) 
{
	...
	g_pDevice->GetSkinManager()->AddTexture(z, "gatea.dds", true, 1.0f, NULL, 0);

	// external cube gate
	CreateCube(YVector(0,0,0), 20.0f, 20.0f, 20.0f, vert, inds, false);	
	g_pDevice->GetVertexManager()->CreateStaticBuffer(VID_UU, z, 24, 36, vert, inds, &g_sGate);

	// internal cube gate
	CreateCube(YVector(0,0,0), 20.0f, 20.0f, 20.0f, vert, inds, true);
	g_pDevice->GetVertexManager()->CreateStaticBuffer(VID_UU, z, 24, 36, vert, inds, &g_sGate2);
				...
				VERTEX *verts, WORD *indices, int *nIndis, int *nVerts)
{
	...
	*nIndis = numTris * 3;
	...
	UINT z = -99;
	g_pDevice->GetSkinManager()->AddSkin(&ambient, &diff, &diff, &spec, fSpec, &z);
	g_pDevice->GetSkinManager()->AddTexture(z, "ground0.dds", false, 0.0f, NULL, 0);

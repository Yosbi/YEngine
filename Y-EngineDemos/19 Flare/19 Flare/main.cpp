...
HRESULT ProgramTick(void) 
{
	...
	g_pDevice->DrawText(g_nFontID, 55, 20, 0, 200, 0, "Y-Engine Demo: Flare Demo");
...
HRESULT Render(float dt)
{
	...
	g_pDevice->SetViewLookAt(YVector(0.0f , 0.0f,-200), YVector(0,0,0), YVector(0,1,0));

	YMatrix wmat;
	wmat.RotaX(-3.1416f/2);
	g_pDevice->SetWorldTransform(&wmat);



	// Set the angle to rotate textures
	static float angle0 = 0;
	angle0 +=6.0f * dt;//(0.25 * dt * 3.14)/180;
	static float angle1 = 0;
	angle1 += 2.5f * dt;
	
	// Set the offsets to the FX
	g_pDevice->SetFXConstant(DAT_FLOAT, "gTexOffset0", &angle0, 0);
	g_pDevice->SetFXConstant(DAT_FLOAT, "gTexOffset1", &angle1, 0);
...
{	
	 return g_pDevice->CreateFX("Flare.fx", "FlareTech", true, NULL);		
} 
				...
				VERTEX *verts, WORD *indices, int *nIndis, int *nVerts)
{
	...
	*nIndis = numTris * 3;
	...
	g_pDevice->GetSkinManager()->AddTexture(z, "flare.dds", false, 0.0f, NULL, 0);
	g_pDevice->GetSkinManager()->AddTexture(z, "flarealpha.bmp", false, 0.0f, NULL, 0);

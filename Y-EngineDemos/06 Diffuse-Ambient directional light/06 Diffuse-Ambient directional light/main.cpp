...
HRESULT ProgramTick(void) 
{
	...
	g_pDevice->DrawText(g_nFontID, 30, 20, 0, 200, 0, "Y-Engine Demo: Diffuse-Ambient Directional light");
...
HRESULT Render(float TimeElapsed)
{
    ...
    g_pDevice->SetViewLookAt(YVector(0,3, -3) * vmat, YVector(0,0,0), YVector(0,1,0));
...
{		
	 return g_pDevice->CreateFX("DiffuseAmbientDir.fx", "AmbientDiffuseTech", true, NULL);		
} 
...
{
	YCOLOR dMtrl = { 0.0f, 0.8f, 0.0f, 1.0f };
	YCOLOR aMtrl = { 0.0f, 0.8f, 0.0f, 1.0f };
	

	UINT    s=-99;
	if(FAILED(g_pDevice->GetSkinManager()->AddSkin(&aMtrl, &dMtrl, &aMtrl, &aMtrl, 1, &s)))
	{}

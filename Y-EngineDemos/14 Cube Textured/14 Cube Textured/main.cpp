...
HRESULT ProgramTick(void) 
{
	...
	g_pDevice->DrawText(g_nFontID, 55, 20, 0, 200, 0, "Y-Engine Demo: Cube Textured (mipmaps)");
...
HRESULT Render(float TimeElapsed)
{
	...
	// back & forwardç
	static float back = -30.0f;
	if (g_pYInputDevice->IsPressed(IDV_KEYBOARD, YVK_D))
	{
		back +=0.1f;
		if (back > -10.0f)
			back = -10.0f;
	}
	if (g_pYInputDevice->IsPressed(IDV_KEYBOARD, YVK_A))
	{
		back -=0.1f;
		if (back < -80.0f)
			back = -80.0f;
    ...
    g_pDevice->SetViewLookAt(YVector(0,up, back) * vmat, YVector(0,0,0), YVector(0,1,0));
...
HRESULT BuildGeometry(void) 
{
	...
	YCOLOR s = {0.8f, 0.8f, 0.8f, 1.0f};

	g_pDevice->GetSkinManager()->AddSkin(&a, &d, &d, &s, 8.0f, &sk);
	g_pDevice->GetSkinManager()->AddTexture(sk, "crate.jpg",false,0,NULL,0);

	CreateCube(YVector(0,0,0), 10.0f, 10.0f, 10.0f, v, i, false);
	g_pDevice->GetVertexManager()->CreateStaticBuffer(VID_UU, sk, 24, 36, v, i, NULL);
	return Y_OK;
} 
